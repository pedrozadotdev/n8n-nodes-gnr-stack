...
class ConnectionPoolManager {
	 ...
	 */
	private makeKey(credentials: RedisCredential): string {
		// The credential contains decrypted secrets, that's why we hash it.
		return createHash('sha1').update(JSON.stringify(credentials)).digest('base64');
	}
	...
	async getConnection<T>(options: GetConnectionOption<T>): Promise<T> {
		const key = this.makeKey(options.credentials);
